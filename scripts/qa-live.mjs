    ...
    for (const pageName of pages) {
      ...
      const state = await page.evaluate(() => {
        ...
        return {
          ...
          headerRect: headerRect ? { left: headerRect.left, right: headerRect.right, top: headerRect.top } : null,
      ...
      record(pageName, viewport, "Required bilingual notice deployed", state.noticeCount === 1 && state.noticeText.includes("Important Test Information") && state.noticeText.includes("중요 안내") && state.noticeText.includes("false-positive") && state.noticeText.includes("위양성"), JSON.stringify({ count: state.noticeCount, textLength: state.noticeText.length }));
      record(pageName, viewport, "Live notice clears header", Boolean(state.noticeRect) && Math.abs(state.noticeRect.top) <= 1 && Boolean(state.headerRect) && state.headerRect.top >= state.noticeRect.bottom + 4, JSON.stringify({ notice: state.noticeRect, header: state.headerRect }));
      record(pageName, viewport, "Live notice height synchronized", Boolean(state.noticeRect) && Math.abs(state.noticeHeightVar - state.noticeRect.height) <= 1.5 && state.bodyPaddingTop >= state.noticeRect.height - 1.5, JSON.stringify({ notice: state.noticeRect, cssVar: state.noticeHeightVar, bodyPaddingTop: state.bodyPaddingTop }));
