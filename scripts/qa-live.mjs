...
try {
  for (const viewport of viewports) {
    ...
    for (const pageName of pages) {
      ...
      record(pageName, viewport, "Required page-language notice deployed", state.noticeCount === 1 && noticeLanguageOk, JSON.stringify({ count: state.noticeCount, textLength: state.noticeText.length }));
